...
class LabelRepository(Generic[T]):
    ...
    def __init__(
        self, client: BaseRepository, label_class: Type[T], resource_type: str
    ):
        self._client = client
    ...
    def create(self, project_id: int, label: T) -> T:
        ...
        """
        resource = (
            f"projects/{project_id}/examples/{label.example}/{self._resource_type}"
        )
...
CategoryRepository = functools.partial(
    LabelRepository[Category], label_class=Category, resource_type="categories"
)
SpanRepository = functools.partial(
    LabelRepository[Span], label_class=Span, resource_type="spans"
)
RelationRepository = functools.partial(
    LabelRepository[Relation], label_class=Relation, resource_type="relations"
)
SegmentRepository = functools.partial(
    LabelRepository[Segment], label_class=Segment, resource_type="segments"
)
TextRepository = functools.partial(
    LabelRepository[Text], label_class=Text, resource_type="texts"
)
BoundingBoxRepository = functools.partial(
    LabelRepository[BoundingBox], label_class=BoundingBox, resource_type="bboxes"
)
